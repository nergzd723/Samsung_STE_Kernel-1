...
static unsigned int freq_to_timer_rate(unsigned int freq)
{
	...
	ret_round = jiffies_to_usecs(usecs_to_jiffies(timer_rates[i]));
	if (timer_rates[i] != ret_round)
...
static void cpufreq_interactive_timer(unsigned long data)
{
	...
	boosted = now < (last_input_time + input_boost_ms * 1000);
	...
	if (pcpu->target_freq >= pcpu->policy->max
		...
		< max(max_freq_hysteresis, pcpu->min_sample_time)) {
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->timer_rate = freq_to_timer_rate(policy->cur);
			pcpu->timer_slack_val = freq_to_timer_slack(policy->cur);
			pcpu->min_sample_time = freq_to_min_sample_time(policy->cur);
